...
// Player panel wrapper IDs in board order: Red, Blue, Yellow, Green
const PANEL_WRAPS = ['p-red-wrap','p-blue-wrap','p-yellow-wrap','p-green-wrap'];
...
// NEW: show name-entry screen with correct number of input cards visible
function goToNames(count){
  selectedPlayerCount = count;

  // Show/hide name cards based on player count
  const cards = ['card-p1','card-p2','card-p3','card-p4'];
  cards.forEach((id, i) => {
    document.getElementById(id).style.display = i < count ? 'flex' : 'none';
  });
...
// ── START GAME ────────────────────────────────────────
function startGame(ai, count){
  vsAI = ai;
  lastVsAI = ai;
  totalPlayers = count || selectedPlayerCount || 4;

  if(ai){
    // Player vs Computers always uses 4 players
    totalPlayers = 4;
  ...
  // Show/hide player panels based on totalPlayers
  // Board layout: Red(0)=top-left, Blue(1)=top-right, Yellow(3)=bottom-left, Green(2)=bottom-right
  // Active players for 2P: Red + Green | for 3P: Red + Blue + Green | for 4P: all
  const activeMap = {
    2: [0, 2],       // Red, Green (diagonal — most balanced 2-player)
    3: [0, 1, 2],    // Red, Blue, Green
    4: [0, 1, 2, 3]  // All
  };
  const activePlayers = activeMap[totalPlayers] || activeMap[4];

  PANEL_WRAPS.forEach((wrapId, i) => {
    const wrap = document.getElementById(wrapId);
    if(wrap) wrap.style.display = activePlayers.includes(i) ? 'flex' : 'none';
  });

  // Build tokens — inactive players have all tokens permanently at step 57 (done/invisible)
  tokens = [];
  for(let pi = 0; pi < 4; pi++){
    if(activePlayers.includes(pi)){
      tokens.push([{step:-1},{step:-1},{step:-1},{step:-1}]);
    } else {
      // Mark inactive player tokens as permanently finished so they're skipped
      tokens.push([{step:57},{step:57},{step:57},{step:57}]);
    }
  }

  cur = activePlayers[0]; // Start with first active player
  rolled = false; diceVal = 0; gameOver = false;
  clearLog(); buildBoard(); render(); updatePanels();
  setStatus(NAMES[cur] + ' ki baari!');
...
// NEW: get next active player index (skips inactive/finished players)
function nextActivePlayer(pi){
  const activeMap = { 2:[0,2], 3:[0,1,2], 4:[0,1,2,3] };
  const active = activeMap[totalPlayers] || activeMap[4];
  const idx = active.indexOf(pi);
...
function afterMove(pi){
  ...
  cur = nextActivePlayer(pi); // Use nextActivePlayer instead of simple modulo
  animateTurnChange();
...
function aiTurn(pi){
  ...
  if(!anyCanMove(pi)){
    ...
    rolled = false;
    setTimeout(() => {
      cur = nextActivePlayer(pi);
      updatePanels(); setStatus(NAMES[cur]+' chance!'); resetTimer(); render();
      if(vsAI && cur !== 0) setTimeout(() => aiTurn(cur), 800);
    }, 1000);
    return;
  }
...
function updatePanels(){
  for(let i = 0; i < 4; i++){
    ...
    panel.classList.remove('active','current-turn');
    if(i === cur){
      panel.classList.add('active','current-turn');
    }
