document.addEventListener('DOMContentLoaded', () => {
  ...
  const citiesConfig = {
    ...
    sydney: {
      ...
      sublabel: 'AEST / AEDT',
      banner: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 50%, #f59e0b 100%)',
      lat: -33.8688,
      lon: 151.2093,
    },
    paris: {
      ...
      sublabel: 'CET / CEST',
      banner: 'linear-gradient(135deg, #2e1065 0%, #7c3aed 60%, #f472b6 100%)',
      lat: 48.8566,
      lon: 2.3522,
    },
    dubai: {
      ...
      timeZone: 'Asia/Dubai',
      sublabel: 'GST',
      banner: 'linear-gradient(135deg, #111827 0%, #374151 40%, #d97706 100%)',
      lat: 25.2048,
      lon: 55.2708,
    },
    mumbai: {
    ...
    rio: {
      ...
      sublabel: 'BRT',
      banner: 'linear-gradient(135deg, #064e3b 0%, #10b981 50%, #fbbf24 100%)',
      lat: -22.9068,
      lon: -43.1729,
    },
    cairo: {
      ...
      sublabel: 'EET',
      banner: 'linear-gradient(135deg, #451a03 0%, #9a3412 50%, #f59e0b 100%)',
      lat: 30.0444,
      lon: 31.2357,
    },
    singapore: {
      ...
      sublabel: 'SGT',
      banner: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #06b6d4 100%)',
      lat: 1.3521,
      lon: 103.8198,
    },
    la: {
      ...
      sublabel: 'PST / PDT',
      banner: 'linear-gradient(135deg, #312e81 0%, #4338ca 40%, #fb7185 100%)',
      lat: 34.0522,
      lon: -118.2437,
    },
  };
  ...
  const timezoneSectors = [
    ...
    },
    {
      ...
      cities: 'Halifax, Caracas, Santiago',
    },
    {
      offset: -3,
      id: 'America/Sao_Paulo',
      name: 'Brasilia Time (BRT)',
      cities: 'Rio de Janeiro, Buenos Aires',
    },
    {
      offset: -2,
      id: 'America/Noronha',
    ...
    },
    {
      ...
      cities: 'Praia, Ponta Delgada',
    },
    {
      offset: 0,
      id: 'Europe/London',
      name: 'Greenwich Mean Time (GMT)',
      cities: 'London, Dublin, Reykjavik',
    },
    {
      offset: 1,
      id: 'Europe/Paris',
    ...
    },
    {
      ...
      cities: 'Cairo, Johannesburg, Athens',
    },
    {
      offset: 3,
      id: 'Europe/Moscow',
      name: 'Moscow Standard Time (MSK)',
      cities: 'Moscow, Baghdad, Nairobi',
    },
    {
      offset: 4,
      id: 'Asia/Dubai',
      name: 'Gulf Standard Time (GST)',
      cities: 'Dubai, Baku, Tbilisi, Muscat',
    },
    {
      offset: 5,
      id: 'Asia/Karachi',
      name: 'Pakistan Standard Time (PKT)',
      cities: 'Karachi, Yekaterinburg',
    },
    {
      offset: 5.5,
      id: 'Asia/Kolkata',
    ...
    },
    {
      ...
      cities: 'Tokyo, Seoul, Yakutsk',
    },
    {
      offset: 10,
      id: 'Australia/Sydney',
      name: 'Australian Eastern Time (AEST)',
      cities: 'Sydney, Melbourne, Vladivostok',
    },
    {
      offset: 11,
      id: 'Pacific/Guadalcanal',
      name: 'Solomon Islands Time (SBT)',
      cities: 'Honiara, Noumea',
    },
    {
      offset: 12,
      id: 'Pacific/Auckland',
      name: 'New Zealand Standard Time (NZST)',
      cities: 'Auckland, Suva',
    },
  ];
  ...
  function renderSearchResults(searchTerm = '') {
    ...
    matches.forEach((city) => {
      const isPinned = activePinnedClocks.includes(city.id);

      const item = document.createElement('div');
      item.className = 'search-city-item';

      item.innerHTML = `
            <div class="search-city-info">
                <h4>${city.name}</h4>
                <p>${city.timeZone}</p>
            </div>
<button
...
>
    ...
    citySearchResults
      ...
      .forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          renderSearchResults(citySearchInput.value);
        });
      });
  }
  ...
  function animateClocks() {
    ...
    if (localClockObj.hourHand)
      localClockObj.hourHand.style.transform = `rotate(${localHr * 30}deg)`;
    if (localClockObj.minuteHand)
      localClockObj.minuteHand.style.transform = `rotate(${localMin * 6}deg)`;
    if (localClockObj.secondHand)
      localClockObj.secondHand.style.transform = `rotate(${localSec * 6}deg)`;
    ...
    const localDateText = now.toLocaleDateString(undefined, {
      weekday: 'short',
      month: 'short',
      day: 'numeric',
    });
    ...
    dynamicClocks.forEach((clock) => {
      ...
      const dateText = targetTime.toLocaleDateString('en-US', {
        weekday: 'short',
        month: 'short',
        day: 'numeric',
      });
  ...
  function initMapInteractions() {
    ...
    const meridian = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'line'
    );
    ...
    if (!markersGroup) {
      markersGroup = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'g'
      );
    ...
    function updateTooltipTickingTime(timeZone) {
      ...
      const tick = () => {
        ...
        try {
          ...
          tooltipDate.textContent = now.toLocaleDateString(undefined, {
            weekday: 'short',
            month: 'short',
            day: 'numeric',
          });
    ...
    sectors.forEach((sector) => {
      sector.addEventListener('mouseover', (e) => {
        ...
        tooltipOffset.textContent =
          (offsetVal >= 0 ? '+' : '') + offsetVal + ' Hrs';
        tooltipTzId.textContent = tz;
        updateTooltipTickingTime(tz);

        // Find matching configurations if any to pin
        const configMatch = Object.values(citiesConfig).find(
          (c) => c.timeZone === tz
        );
        if (configMatch) {
  ...
  fetch('world-map.svg')
    ...
    .catch((err) => {
      console.warn(
        'Could not load external SVG world map (likely CORS/file protocol restriction). Falling back to inline high-end minimalist map.',
        err
      );
